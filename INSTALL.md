...
It describes version 1.23.0. The Python package, the server, and the bundled Chrome
...
On Windows the generator points `"command"` at the checkout's `pythonw.exe` instead
of `python.exe`: a console interpreter under a windowed MCP client owns a visible
console for the whole session, while `pythonw` runs the same stdio pipes with no
console at all. If LM Studio cannot find the virtual environment, either launch
LM Studio with that environment on `PATH` or use the system Python where you installed `requirements.txt`.
...
Leave **Web Search Neo Companion** enabled. Its toolbar popup (also on
**Alt+Shift+N**) is a compact icon-first widget showing live status — connected,
connecting, waiting with the retry countdown, disabled, or error — the
controlled-tab count against the parallel-session limit, the bridge endpoint,
a GitHub release/version check and repository link, a persistent on/off switch,
**Reconnect**, and **Release tabs**.
...
The bundled companion is version 1.23.0 and declares five permissions: `alarms`, `debugger`,
...
ID. If that port conflicts, set `WEB_SEARCH_NEO_BRIDGE_PORT` for the MCP and enter the
same number under **Settings → Bridge port** in the companion's popup. The companion
stores it, reconnects immediately, and dials the same port after a browser restart —
no source edit and no extension reload.

The companion forwards only the DevTools methods this MCP actually issues; any other
protocol method is refused inside the extension, so an authenticated local peer holds
the contract's capabilities rather than the whole protocol.
...
3. Check the card's version. It must read 1.23.0; anything older speaks bridge protocol 1
...
Nothing needs clicking. If the logon launcher (`scripts/install_bridge_autostart.bat`,
run once) is in place, the bridge is already listening; otherwise the first MCP run
starts a daemon on demand. Opening Chrome then reconnects the companion by itself and
its badge turns `ON` within seconds. If it reads `OFF`, press **Reconnect** in the popup —
and **Reload** the card first when the folder Chrome loads is not the one
`setup_current_chrome` reports in `extension_directory`.
